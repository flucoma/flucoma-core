    ...
    bool checkBounds(const fluid::FluidTensorSlice<N> &slice, Dims... dims) {
      ...
      return std::equal(indexes, indexes + N, slice.extents.begin(),
        ...
        template <typename T, size_t N> struct FluidTensorInit {
          using type =
              std::initializer_list<typename FluidTensorInit<T, N - 1>::type>;
        };
        ...
        template <typename T>
        struct FluidTensorInit<T,0>; //undefined on purpose: things should barf if this happens
...
//        size_t doSliceDim(const fluid::FluidTensorSlice<M>& original_slice, fluid::FluidTensorSlice<M>& new_slice, size_t n)
//        {
//            return doSliceDim<N>(new_slice, fluid::slice(n, 1, 1));
//        }
//
//        template<size_t N, size_t M, typename T>
//        size_t doSliceDim(const fluid::FluidTensorSlice<M>& original_slice, fluid::FluidTensorSlice<M>& new_slice, const T& s)
//        {
...
//        }
        ...
        struct SliceIterator
        {
            ...
            SliceIterator(const FluidTensorSlice<N> &s, pointer base,
                          bool end = false)
                : mDesc(s), mBase(base) {
...
//            explicit FluidTensor_dimIterator(const fluid::FluidTensorBase<T,N+1>& obj):container(obj){}
//
//            //dereferenceable
...
//        };//FluidDimIterator
    ...
    private:
        ...
        size_t doSliceDim(const fluid::FluidTensorSlice<M> &newSlice,
                          size_t n) {
