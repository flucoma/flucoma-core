...
public:
  ...
  static const std::vector<client::Descriptor> &getParamDescriptors() {
    ...
    if (params.empty()) {
      ...
      params.emplace_back("hsize", "Harmonic Filter Size", client::Type::kLong);
  ...
  **/
  std::tuple<bool, std::string, ProcessModel> sanityCheck() {
    ...
    client::BufferAdaptor::Access harmBuf(
        client::lookupParam("harmbuf", getParams()).getBuffer());
    client::BufferAdaptor::Access percBuf(
        client::lookupParam("percbuf", getParams()).getBuffer());
    ...
    if (pSize > (fftSize.getLong() / 2) + 1) {
      return {false,
              "Percussive filter can not be bigger than fft size / 2 + 1, and "
              "should really be smaller",
              model};
    }
    ...
    if (model.mode > 0) {
      ...
      if (pf1 >= pf2) {
        return {
            false,
            "Percussive Threshold low frequency must be below high frequency",
            model};
      }
      ...
      if (hf1 >= hf2) {
        return {false,
                "Harmonic Threshold low frequency must be below high frequency",
                model};
      }
  ...
  void process(ProcessModel model) {
    ...
    algorithm::RTHPSS processor(
        model.fftsize / 2 + 1, model.percsize, model.harmsize, model.mode,
        model.hThreshFreq[0], model.hThreshAmp[0], model.hThreshFreq[1],
        model.hThreshAmp[1], model.pThreshFreq[0], model.pThreshAmp[0],
    ...
    for (size_t i = 0; i < model.channels; ++i) {
      ...
      //          auto isanan =
      //          std::find_if(spectrum.mData.begin(),spectrum.mData.end(),[](std::complex<double>
      //          d){
      ...
      FluidTensor<std::complex<double>, 2> harmonicSpec(spectrum.mData.rows(),
                                                        spectrum.mData.cols());
      FluidTensor<std::complex<double>, 2> percussiveSpec(
          spectrum.mData.rows(), spectrum.mData.cols());
      ;
      FluidTensor<std::complex<double>, 2> residualSpec(spectrum.mData.rows(),
                                                        spectrum.mData.cols());
      ;
