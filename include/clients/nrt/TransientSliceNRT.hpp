  ...
  **/
  std::tuple<bool, std::string, ProcessModel> sanityCheck() {
    ...
    if (bufCount > uniqueBuffers.size()) {
      return {false, "Buffers are the same. They all need to be distinct",
              model};
    }
  ...
  TransientSliceNRT() { newParameterSet(); }
  ...
  void process(ProcessModel model) {
    ...
    for (size_t i = 0; i < model.channels; ++i) {
      monoSource.apply(src.samps(i), [](double &x, double y) { x += y; });
    }

    algorithm::TransientSegmentation segmentor(model.order, model.iterations,
                                               model.robustFactor);
    segmentor.prepareStream(model.blocksize, model.padding);
    segmentor.setDetectionParameters(model.skew, model.fwdThresh,
    ...
    for (size_t i = 0; i < model.frames; i += hopsize) {
      ...
      FluidTensorView<const double, 1> markers(
          segmentor.process(monoSource.data() + i, size), 0, hopsize);
