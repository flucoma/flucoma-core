    ...
    public:
      ...
      static const std::vector<client::Descriptor>& getParamDescriptors()
      {
        ...
        if(params.empty())
        {
          ...
          params.emplace_back(desc_type{"transbuf", "Transients Buffer",
                                        client::Type::kBuffer});
...
//        client::BufferAdaptor* src= params[0].getBuffer();
        ...
        model.halfWindow = std::round(client::lookupParam("windowsize", mParams).getFloat() / 2);
      ...
      void process(ProcessModel model)
      {
        ...
        for(size_t i = 0; i < model.channels; ++i)
        {
          ...
          algorithm::TransientExtraction extractor(model.order, model.iterations, model.robustFactor, false);
