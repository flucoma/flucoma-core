...
public:
  ...
  static const std::vector<client::Descriptor> &getParamDescriptors() {
    ...
    if (params.empty()) {
      ...
      params.emplace_back(
          desc_type{"transbuf", "Transients Buffer", client::Type::kBuffer});
      params.back().setInstantiation(false);

      params.emplace_back(
          desc_type{"resbuf", "Residual Buffer", client::Type::kBuffer});
      params.back().setInstantiation(false);

      // The main blocking parameters (expose in ms for the block and pad,
      // possiby also model order as the meaning is SR dependant)

      //          int paramOrder = 200;       // The model order (higher ==
      //          better quality + more CPU - should be quite a bit smaller than
      //          the block size)
      ...
      params.back().setInstantiation(false).setMin(-10).setMax(10).setDefault(
          0);
      ...
      //          // This is broken right now - SET FALSE AND DO NOT EXPOSE -
      //          turning it on will produce worse results (should make things
      //          better)
      //
      //          const bool paramRefine = false;
      //
      //          // These parameters relate to the way we estimate the AR
      //          parameters robustly from data
      //          // They probably aren't worth exposing to the end user (DO NOT
      //          EXPOSE THE ROBUST FACTOR)
      //
      //          int paramIterations = 3;           // How many times to
      //          iterate over the data to robustify it (can be 0) double
      //          paramRobustFactor = 3.0;    // Data futher than this *
      //          deviation from the expected value it will be clipped
    }
    return params;
  }
  ...
  /**
   ...
   Args
   source buffer [offset numframes offsetchan numchan] transient_buff
  residual_buff
  ...
  **/
  std::tuple<bool, std::string, ProcessModel> sanityCheck() {
    ...
    //        client::BufferAdaptor* src= params[0].getBuffer();
    ...
    client::Instance &transBufParam = mParams[5];
    client::BufferAdaptor::Access transBuf(transBufParam.getBuffer());
    ...
    client::Instance &resBufParam = mParams[6];
    client::BufferAdaptor::Access resBuf(resBufParam.getBuffer());
    ...
    model.skew = std::pow(2, client::lookupParam("skew", mParams).getFloat());

    model.padding = client::lookupParam("padding", mParams).getLong();
    model.fwdThresh = client::lookupParam("threshfwd", mParams).getFloat();
    model.backThresh = client::lookupParam("threshback", mParams).getFloat();
    ;
    model.debounce = client::lookupParam("debounce", mParams).getFloat();
    ;
  ...
  TransientNRTClient() { newParameterSet(); }

  void process(ProcessModel model) {
    client::BufferAdaptor::Access src(model.src);
    client::BufferAdaptor::Access trans(model.trans);
    ...
    for (size_t i = 0; i < model.channels; ++i) {
      ...
      srcFrames(fluid::Slice(0, model.frames)) =
          src.samps(model.offset, model.frames, model.channelOffset + i);
      algorithm::TransientExtraction extractor(model.order, model.iterations,
                                               model.robustFactor, false);
      extractor.prepareStream(model.blocksize, model.padding);
      extractor.setDetectionParameters(model.skew, model.fwdThresh,
                                       model.backThresh, model.halfWindow,
                                       model.debounce);
      ...
      for (size_t j = 0; j < model.frames; j += hopsize) {
        ...
        extractor.extract(transientFrames.data() + j, cleanFrames.data() + j,
                          srcFrames.data() + j, size);
      }
