...
public:
  ...
  **/
  ...
  void process(ProcessModel model) {
    ...
    algorithm::SineExtraction processor(model.winsize, model.fftsize,
                                        model.hopsize, model.bandwidth,
                                        model.threshold, model.minTrackLength,
                                        model.magWeight, model.freqWeight);

    for (size_t i = 0; i < model.channels; ++i) {
      FluidTensor<double, 1> input(
      ...
      sum.apply(decomposition.noise, [](double &x, double &n) { x += n; });
