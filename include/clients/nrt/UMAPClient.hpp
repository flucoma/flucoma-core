...
public:
  ...
  enum { kNumDimensions, kNumNeighbors, kMinDistance, kMaxIter, kLearningRate };

  FLUID_DECLARE_PARAMS(
      LongParam("numDimensions", "Target Number of Dimensions", 2, Min(1)),
      LongParam("numNeighbours", "Number of k Nearest Neighbours", 15, Min(1)),
      FloatParam("minDist", "Minimum Distance", 0.1, Min(0)),
      LongParam("maxIter", "Maximum Number of Iterations", 200, Min(1)),
  ...
  MessageResult<void> fitTransform(DataSetClientRef sourceClient,
    ...
    result =
        mAlgorithm.process(src, get<kNumNeighbors>(), k, get<kMinDistance>(),
                           get<kMaxIter>(), get<kLearningRate>());
