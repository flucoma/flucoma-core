...
public:
  static const std::vector<client::Descriptor> &getParamDescriptors() {
    ...
    if (params.size() == 0) {
      ...
      params.emplace_back("filterbuf", "Filters Buffer", client::Type::kBuffer);
      ...
      params.emplace_back("iterations", "Iterations", client::Type::kLong);
      params.back()
  ...
  void reset() {
    ...
    size_t iterations =
        client::lookupParam("iterations", getParams()).getLong();
    mNMF =
        std::unique_ptr<algorithm::NMF>(new algorithm::NMF(mRank, iterations));
  ...
  std::tuple<bool, std::string> sanityCheck() {
    ...
    client::BufferAdaptor::Access filters(
        client::lookupParam("filterbuf", getParams()).getBuffer());
    //        client::BufferAdaptor::Access output(client::lookupParam("outbuf",
    //        getParams()).getBuffer());
    ...
    if (filters.numChans() != rank ||
        filters.numFrames() != ((fftSize.getLong() / 2) + 1)) {
      return {false, "Filters buffer needs to be (fftsize / 2 + 1) frames by "
                     "rank channels"};
    }
    return {true, "Groovy"};
  }
  ...
  void process(data_type input, data_type output) override {
    if (filbuf) {
      ...
      tmpMagnitude.apply(mSTFT->processFrame(input.row(0)),
                         [](double &x, std::complex<double> &y) -> double {
                           x = std::abs(y);
                         });
      ...
      //          std::transform(tmpOut.begin(), tmpOut.end(), tmpOut.begin(),
      //          [&](double& x)->double{
