...
public:
  ...
  void reset() {
    ...
    mHPSS = std::unique_ptr<algorithm::RTHPSS>(new algorithm::RTHPSS(
        nBins, pBins, hBins, mode(), hthreshF1, hthreshA1, hthreshF2, hthreshA2,
        pthreshF1, pthreshA1, pthreshF2, pthreshA2));
  ...
  std::tuple<bool, std::string> sanityCheck() {
    ...
    if (pSize > (fftSize.getLong() / 2) + 1) {
      return {false, "Percussive filter can not be bigger than fft size / 2 + "
                     "1, and should really be smaller"};
    }
  ...
  void process(data_type input, data_type output) override {
    ...
    if (mode() > 0) {
      mHPSS->setHThresholdX1(
          client::lookupParam("htf1", getParams()).getFloat());
      mHPSS->setHThresholdY1(
          client::lookupParam("hta1", getParams()).getFloat());
      mHPSS->setHThresholdX2(
          client::lookupParam("htf2", getParams()).getFloat());
      mHPSS->setHThresholdY2(
          client::lookupParam("hta2", getParams()).getFloat());
    }

    if (mode() == 2) {
      mHPSS->setPThresholdX1(
          client::lookupParam("ptf1", getParams()).getFloat());
      mHPSS->setPThresholdY1(
          client::lookupParam("pta1", getParams()).getFloat());
      mHPSS->setPThresholdX2(
          client::lookupParam("ptf2", getParams()).getFloat());
      mHPSS->setPThresholdY2(
          client::lookupParam("pta2", getParams()).getFloat());
    }
