...
namespace client {
    ...
    public:
        ...
        public:
            ...
            void copyFrom(VectorView dst, size_t srcOffset,
                          size_t size) override {
              std::copy(mSig + srcOffset, mSig + srcOffset + size,
                        dst.begin());
            }

            void copyTo(VectorView src, size_t dstOffset,
                        size_t size) override {
...
//        BaseAudioClient& operator=(BaseAudioClient&)= delete;
        ...
        BaseAudioClient(size_t maxFrameSize, size_t nChannelsIn = 1,
                        size_t nChannelsOut = 1,
                        size_t nIntermediateChannels = 0)
            : mMaxFrameSize(maxFrameSize), mChannelsIn(nChannelsIn),
              mChannelsOut(nChannelsOut),
              mIntermediateChannels(
                  nIntermediateChannels ? nIntermediateChannels : nChannelsOut),
      ...
      void doProcess(InputIt input, InputIt inend, OutputIt output,
        ...
        mFrameTime = mFrameTime < mHostBufferSize
                           ? mFrameTime
