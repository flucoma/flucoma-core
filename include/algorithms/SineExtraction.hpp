...
using ArrayXXdMap =
    Map<const Eigen::Array<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>>;
using ArrayXdMap = Map<const Eigen::Array<double, Eigen::Dynamic, Eigen::RowMajor>>;
...
private:
  ...
  ArrayXd computeWindowTransform(vector<double> window) {
    ...
    ArrayXcd transform = mFFT.process(Map<const ArrayXd>(window.data(), mWindowSize));
