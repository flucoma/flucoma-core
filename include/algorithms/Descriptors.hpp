...
class Descriptors {
  ...
  struct Pow3 {
    double operator()(double a) { return a * a * a; }
  };
  struct Pow4 {
    double operator()(double a) { return Pow2()(Pow2()(a)); }
  };
  struct Log {
    double operator()(double a) { return log(a); }
  };
  struct Abs {
    double operator()(double a) { return fabs(a); }
  };
  ...
  struct AbsDifference {
    double operator()(double a, double b) { return fabs(a - b); }
  };
  ...
  struct KLDifference {
    double operator()(double a, double b) { return a * log(a / b); }
  };
  struct SKLDifference {
    double operator()(double a, double b) { return (a - b) * log(a / b); }
  };
  struct MKLDifference {
    double operator()(double a, double b) { return log(a / b); }
  };

  // Index difference from a fixed reference

  template <typename Op> struct IndexDiff {
...
public:
  ...
  static double skewness(const Real &input) {
    ...
    return statIndexWeightedSum(input, IndexDiff<Pow3>(c)) /
           (sN * sN * sN * statSum(input));
  }

  static double kurtosis(const Real &input) {
    double c = centroid(input);
    double sN = spread(input);
    return statIndexWeightedSum(input, IndexDiff<Pow4>(c)) /
           (sN * sN * statSum(input));
  }
  ...
  static double RMS(const Real &input) { return sqrt(statMeanSquares(input)); }

  static double crest(const Real &input) {
    return statMax(input, Abs()) / RMS(input);
  }

  static double peak(const Real &input) { return statMax(input, Abs()); }

  // HFC

  // If handed a spectra returns the HFC with positions scaled in bins

  static double HFC(const Real &input) { return statIndexWeightedSum(input); }
  ...
  // Remove values from two vectors in positions that are not greater in the
  // first vector than the second

  static void forwardFilter(Real &vec1, Real &vec2) {
    assert(vec1.size() == vec2.size() && "Vectors should match in size");

    for (auto it = vec1.begin(), jt = vec2.begin(); it != vec1.end();
         it++, jt++) {
      if (*it < *jt)
        *it = *jt = 0.0;
    }
  ...
  // Log difference between two vectors (if spectra expected as amplitude
  // spectra, not power spectra)
  ...
  static double differenceFT(const Real &vec1, const Real &vec2) {
    ...
    return statSumProduct(vec1, vec2) /
           sqrt(statSumSquares(vec1) * statSumSquares(vec2));
  }

  // Itakura-Saito difference between two vectors (if spectra expected as
  // amplitude spectra, not power spectra)

  static double differenceIS(const Real &vec1, const Real &vec2) {
    return statSum(vec1, vec2, ISDifference());
  }

  // Kullback-Liebler difference between two vectors (if spectra expected as
  // amplitude spectra, not power spectra)
  ...
  // Symmetric Kullback-Liebler difference between two vectors (if spectra
  // expected as amplitude spectra, not power spectra)

  static double differenceSKL(const Real &vec1, const Real &vec2) {
    return 0.5 * statSum(vec1, vec2, SKLDifference());
  }

  // Modified Kullback-Liebler difference between two vectors (if spectra
  // expected as amplitude spectra, not power spectra)
...
private:
  ...
  template <typename Op> static double statSum(const Real &input, Op modifier) {
    double sum = 0.0;
  ...
  static double statSum(const Real &input) { return statSum(input, NOP()); }

  static double statSumSquares(const Real &input) {
    return statSum(input, Pow2());
  }

  static double statSumAbs(const Real &input) { return statSum(input, Abs()); }

  static double statSumLogs(const Real &input) { return statSum(input, Log()); }
  ...
  template <typename Op> static double statMin(const Real &input, Op modifier) {
  ...
  template <typename Op> static double statMax(const Real &input, Op modifier) {
  ...
  static double statPDFPercentile(const Real &input, double centile) {
    double target =
        statSum(input) * std::min(100.0, std::max(centile, 0.0)) / 100.0;
