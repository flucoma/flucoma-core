...
public:
  void init(int dims, const FluidDataset<std::string, double, 1> &source,
            const FluidDataset<std::string, double, 1> &target)
  {
    mDims = dims;
    mSource = &source;
  }
  ...
  void init(int dims, const FluidDataset<std::string, double, 1> &source)
  {
    ...
    void getMeans(RealMatrixView out) { out = _impl::asFluid(mMeans); }
  ...
  private:
    ...
    int mDims{0};
    FluidDataset<std::string, double, 1> *mSource{nullptr};
    FluidDataset<std::string, double, 1> *mTarget{nullptr};
};
} // namespace algorithm
} // namespace fluid
