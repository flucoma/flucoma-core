...
namespace stft {
    ...
    class STFT {
        //Map an Eigen Array
        using input_map = Eigen::Map<const Eigen::Array<double,1,Eigen::Dynamic, Eigen::RowMajor>>;
        using output_map = Eigen::Map<Eigen::Array<std::complex<double>,1,Eigen::Dynamic, Eigen::RowMajor>>;
    public:
        STFT(size_t windowSize, size_t fftSize, size_t hopSize)
        : mWindowSize(windowSize), mHopSize(hopSize), mFrameSize(fftSize / 2 + 1),
        mFFT(fftSize),mSTFTBuffer(mFrameSize), mSTFTOutMap(mSTFTBuffer.data(),1,mFrameSize), mSTFTInMap(nullptr,0)
        {
        ...
        ComplexVectorView process_frame(const RealVectorView& frame)
        {
            const size_t nsamps = frame.size();
//            assert(frame.rows() == 1);
            assert(nsamps == mWindowSize);
            //Point map towards incoming pointer
            new (&mSTFTInMap) input_map(frame.data(), 1, nsamps);
    ...
    class ISTFT {
        using input_map = Eigen::Map<const Eigen::Array<std::complex<double>,1,Eigen::Dynamic, Eigen::RowMajor>>;
        using output_map = Eigen::Map<Eigen::Array<double,1,Eigen::Dynamic, Eigen::RowMajor>>;
    public:
        ISTFT(size_t windowSize, size_t fftSize, size_t hopSize)
        : mWindowSize(windowSize), mHopSize(hopSize), mFrameSize(fftSize / 2 + 1),
        mScale(1 / double(fftSize)), mIFFT(fftSize),
        mBuffer(2,mWindowSize),mOutputMap(mBuffer.row(0).data(),1,mWindowSize), mInputMap(nullptr,0)
        {
            mWindow = Map<ArrayXd>(windowFuncs[WindowType::Hann](mWindowSize).data(),
                                   mWindowSize);
            ...
            mSumSquared = mHopSize /  mWindowSquared.sum();
        }
        ...
        RealMatrixView process_frame(const ComplexVectorView& frame)
        {
            assert(frame.size() == mFrameSize);
//            assert(frame.rows() == 1);
            //Point input map at frame
            new (&mInputMap) input_map(frame.data(), 1, mFrameSize);
            mOutputMap = (mIFFT.process(mInputMap).block(0,0,mWindowSize,1) * mWindow * mScale).transpose();
            return mBuffer;
        }
