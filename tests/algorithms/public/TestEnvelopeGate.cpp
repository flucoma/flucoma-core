...
runTest(FluidTensorView<const double, 1> testSignal, TestParams const& p)
{
  ...
  index latency  = std::max<index>(
          p.minLengthAbove + p.lookback,
...
TEST_CASE("EnvelopeGate is predictable with smooth sine bursts", "[AmpGate][slicers]")
{
  auto sig = testsignals::smoothSine();
  auto expectedOnsets = std::vector<index>{ 1876, 1942, 2009, 2076, 2144, 2212, 2279, 2347, 2415, 2483, 2551, 2619, 2687, 19431, 19501, 19571, 19641, 19711, 19781, 19851, 19921, 19991, 20062, 20133, 20205, 23926, 23992, 24059, 24126, 24194, 24262, 24329, 24397, 24465, 24533, 24601, 24669, 24737, 41481, 41551, 41621, 41691, 41761, 41831, 41901, 41971, 42041, 42112, 42183, 42255 };

  auto expectedOffsets = std::vector<index>{1890, 1965, 2039, 2113, 2185, 2257, 2329, 2400, 2471, 2542, 2613, 2684, 19430, 19497, 19564, 19631, 19698, 19764, 19831, 19897, 19963, 20028, 20092, 20156, 20219, 23940, 24015, 24089, 24163, 24235, 24307, 24379, 24450, 24521, 24592, 24663, 24734, 41480, 41547, 41614, 41681, 41748, 41814, 41881, 41947, 42013, 42078, 42142, 42206, 42269};
...
TEST_CASE("EnvelopeGate is predictable with smooth sine bursts and hysteresis", "[AmpGate][slicers]")
{
  auto sig = testsignals::smoothSine();
...
TEST_CASE("EnvelopeGate is predictable with smooth sine bursts and debouncing", "[AmpGate][slicers]")
{
  auto sig = testsignals::smoothSine();
  auto expectedOnsets = std::vector<index>{1876, 2347, 19431, 19921, 23926, 24397, 41481, 41971};

  auto expectedOffsets = std::vector<index>{2329, 19430, 19897, 20362, 24379, 41480, 41947, 42412 };
...
TEST_CASE("EnvelopeGate is predictable with smooth sine bursts and gap debouncing", "[AmpGate][slicers]")
{
  auto sig = testsignals::smoothSine();
  auto expectedOnsets = std::vector<index>{1876, 2347, 2841, 19871, 23926, 24397, 24891, 41921};

  auto expectedOffsets = std::vector<index>{1890, 2400, 19430, 19897, 23940, 24450, 41480, 41947 };
...
TEST_CASE("EnvelopeGate is predictable with smooth sine bursts and min time above", "[AmpGate][slicers]")
{
  auto sig = testsignals::smoothSine();
...
TEST_CASE("EnvelopeGate is predictable with smooth sine bursts and min time below", "[AmpGate][slicers]")
{
  auto sig = testsignals::smoothSine();
...
TEST_CASE("EnvelopeGate is predictable with smooth sine bursts and lookahead", "[AmpGate][slicers]")
{
  auto sig = testsignals::smoothSine();
  auto expectedOnsets = std::vector<index>{1875, 23925 };
...
TEST_CASE("EnvelopeGate is predictable with smooth sine bursts and lookback", "[AmpGate][slicers]")
{
  auto sig = testsignals::smoothSine();
  auto expectedOnsets = std::vector<index>{1435, 19499, 19568, 19638, 19707, 19776, 19846, 19915, 19985, 20055, 20125, 20195, 23485, 41549, 41618, 41688, 41757, 41826, 41896, 41965, 42035, 42105, 42175, 42245 };

  auto expectedOffsets = std::vector<index>{19496, 19563, 19630, 19697, 19763, 19830, 19896, 19962, 20027, 20091, 20155, 20218, 41546, 41613, 41680, 41747, 41813, 41880, 41946, 42012, 42077, 42141, 42205, 42268 };
...
TEST_CASE("EnvelopeGate is predictable with smooth sine bursts and both lookahead and lookback", "[AmpGate][slicers]")
{
  auto sig = testsignals::smoothSine();
...
TEST_CASE("EnvelopeGate is predictable on a real signal", "[AmpGate][slicers]")
{
  ...
  auto expectedOnsets = std::vector<index>{ 1269, 38394, 69830, 88034, 114533, 151761, 176327, 202300, 220866, 239779, 252598, 276315, 283982, 326755, 353444, 372504, 390197, 417174 };

  auto expectedOffsets = std::vector<index>{ 23328, 65751, 81905, 96307, 124975, 172742, 184798, 216023, 232738, 249671, 272516, 283322, 310995, 343433, 366663, 384191, 398299, 428473 };
