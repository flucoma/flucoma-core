...
/**!
 ...
 __Input parameters of EigenThings should always be refs, to avoid new
 allocations__

 1) EigenThing foo(Ref<const EigenThing>& input) : just return an EigenThing,
 hope that return value optimisation (RVO) will do its thing and compile to a
 move operation 2) Ref<EigenThing> foo(Ref<const EigenThing>& input): return a
 ref 3) void foo(Ref<const EigenThing>& input, Ref<EigenThing>& output)

 Other factors:
 - These are functions (like FFT.process()) that will be called a lot in an
 inner processing loop – So we want a output buffer allocated at class scope,
 rather than every call (how does this jive with RVO?) – Eventually these are
 always going to convert back to FluidTensor types, via EigenMaps. FluidTensors
 are row-major only, Eigen types are column-major by default (and seem to be
 slower in row-major). Does the cost of going column-major to row-major on the
 conversion have an impact? What about for the processing?

 **/

template <typename T, typename U = T> class EigenReturnProfiler {
...
public:
  ...
  void f_output_parameter_passbase(const Eigen::ArrayBase<DerivedIn> &input,
                                   Eigen::ArrayBase<DerivedOut> &output) {
    output = input * m_window;
  }
...
int main(int argc, char *argv[]) {
  ...
  using const_map_type = Eigen::Map<
  ...
  dur = end - start;
  std::cout << "Colwise Preallocation Ref Return Arraybase Template Fn: "
            << dur.count() << '\n';
  ...
  dur = end - start;
  std::cout << "Rowwise Preallocation Ref Return ArrayBase Template Fn: "
            << dur.count() << '\n';
