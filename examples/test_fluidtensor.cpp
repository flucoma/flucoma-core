...
int main(int argc, char* argv[])
{
  ...
  int j = 0;
  for (auto&& i : threebytwo) { std::cout << j++ << ' ' << i << '\n' << '\n'; }

  //  return 0;

  // We can initialize a tensor with some elements using braces:
  fluid::FluidTensor<double, 1> tinit{1.0, 2.0, 3.0};
  std::cout << "Rank: " << tinit.order << " Length: " << tinit.extent(0)
            << " Data: " << tinit << '\n';

  // We can nest them for multiple dimenions
  fluid::FluidTensor<double, 2> tinit2{{1.0, 2.0, 3.0}, {4, 5, 6}};
  std::cout << "Rank : " << tinit2.order << " Dimension 1: " << tinit2.extent(0)
            << " Dimension 2: " << tinit2.extent(1) << " Data: " << tinit2
            << '\n';

  // Three dimensions....
  fluid::FluidTensor<double, 3> threedee{{{1, 2}, {3, 4}}, {{5, 6}, {7, 8}}};
  std::cout << "3D:\n" << threedee << '\n';
  ...
  // Eigen mapping.
  using FluidTensorToEigen = Eigen::Map<
      Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>>;

  // This is an Eigen::Map:
  FluidTensorToEigen maptoflu(tinit2.data(), tinit2.extent(0),
                              tinit2.extent(1));
  ...
  // Map a row
  using FluidTensorToEigenVector =
      Eigen::Map<Eigen::Matrix<double, 1, Eigen::Dynamic, Eigen::RowMajor>>;
  ...
  // Is it right?
  assert(
      std::equal(tinit2Expected.begin(), tinit2Expected.end(), tinit2.data()));
  ...
  // brute force fill
  for (int i = 0; i < x * y; ++i) *(twodeeTest.data() + i) = i;
  ...
  // Use() operator with at least one fluid::slice(start,size,stride), possibly
  // mixed  with integer types to do arbitary slicees:
  ...
  AudioFileData data = readFile(
      "/Applications/Max.app/Contents/Resources/C74/media/msp/jongly.aif");
  ...
  // Data should match
  assert(
      std::equal(data.audio[0].begin(), data.audio[0].end(), audioTest.data()));
  ...
  sin.apply(ramp, [](double& x, double idx) {
    x = std::sin(M_2_PI * idx * 32. *
                 (1024. / 44100.)); // some arbitary sine wave
  });

  cos.apply(ramp, [](double& x, double idx) {
    x = std::cos(M_2_PI * idx * 32. *
                 (1024. / 44100.)); // some arbitary cosine wave
  });
  ...
  win.apply([](double& x) {
    x = 0.5 -
        0.5 * (std::cos(
                  (M_PI * 2 * x) /
                  1024)); // periodic hann window (1024,not 1023 in denominator)
  });
  ...
  // Test for assumption about reading from interleaved structure (e.g
  // multichannel buffers in max and sc)
  fluid::FluidTensorView<double, 2> interT =
      fluid::FluidTensorView<double, 2>({0, {50, 2}}, interleave);
