...
double paramDetectThreshHi = 2.9;        // The threshold for detection (in multiples of the model deviation)
...
int paramDetectHold = 500;               // The hold time for detection (in samples) - CAN (and probably should be higher than for extraction)
...
int main(int argc, const char * argv[])
{
  ...
  TransientSegmentation segmentor(paramOrder, paramIterations, paramRobustFactor);

  segmentor.prepareStream(paramBlockSize, paramPad);
  segmentor.setDetectionParameters(paramDetectPower, paramDetectThreshHi, paramDetectThreshLo, paramDetectHalfWindow, paramDetectHold);
